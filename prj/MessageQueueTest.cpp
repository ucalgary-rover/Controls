...
This is a simple test that checks if MessageQueue works with multiple threads concurrently pushing, popping, and viewing data into and from the MessageQueue

There are producer, consumer, and peeker threads. The producer pushes messages into the MessageQueue, the consumer pops messages from MessageQueue, and the peeker views the front element.

If everything works properly, there should be no errors (duh) and the MessageQueue should be empty by the end of the test. Priority messages should always be popped first.
...
void producer(MessageQueue &queue, int id, int numMessages) {
    for (int i = 0; i < numMessages; ++i) {
        ...
        std::cout << "Producer " << id << " pushed message " << i << " (Priority: " << isPriority << ")\n";
    }
}
...
void peekFront(MessageQueue &queue, int id) {
    try {
        ...
        std::cout << "Consumer " << id << " encountered error while peeking: " << e.what() << "\n";
    }
}
...
int main() {
    ...
    std::cout << "Priority queue size: " << queue.sizePriority() << " (Expected 0 if all messages were processed)\n";
    std::cout << "Regular queue size: " << queue.sizeRegular() << " (Expected 0 if all messages were processed)\n";
    std::cout << "Final queue size: " << queue.size() << " (Expected 0 if all messages were processed)\n";
    return 0;
}
