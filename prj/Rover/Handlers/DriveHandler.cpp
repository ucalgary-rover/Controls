...
Large changes in wheel angle will cause the rover to stop to prevent unexpected movement (> 10 degrees)

Generally, "spinning" is used to refer to the wheels rotating forward or backwards
Generally, "turning" is used to refer to the steppers pointing the wheels in a different direction 
...
assumes wheel stepper motors measure angle from positive y-axis increasing counterclockwise
---> range of motion restricted between 90 and -90

since range of motion limited, all angles given to the wheel must be adjusted to fit within the range

Stepper motors turn wheels and report when their targets are hit afterwards
-----------------> waiting for the wheels to finish turning still needs to be programmed
...
DriveHandler::DriveHandler(Drive& drive, MessageQueue& driveQueue) {
    ...
    // Rover.cpp (NOT A COPY!!!) This allows us to use the same driveQueue in this
...
float DriveHandler::spotTurnSpeed(int stickAngle) {
    ...
    //calculates speed based on the adjustment with the sign determining the direction
...
void DriveHandler::spotTurn(int stickAngle) {
    ...
    float wheelAngle = TO_DEGREES(atan(m_drive.getWidth() / m_drive.getLength()));
    ...
    for (int stepper = 0; stepper < DRIVE_INDEX_WHEEL_COUNT; stepper++) {
        ...
        if (stepper == 0 || stepper == 3) {
            turnWheel(*motorStuct.motorHandle.stepperMotor,
                        -wheelAngle);
...
int DriveHandler::strafeAngleAdjust(int stickTheta) {
    ...
    // since 90 and 270 are equivalent positions, the following code tries to find which is easiest to move to
        ...
        for (int stepper = 0; stepper < DRIVE_INDEX_WHEEL_COUNT; stepper++) {
            ...
            PhidgetStepper_getPosition(*motorStuct.motorHandle.stepperMotor, &currentPos);
            positionSum += currentPos;
        }
        
        positionAverage = (int)(positionSum / DRIVE_INDEX_WHEEL_COUNT);

        //finding differences between current position and possible satisfactory end points
...
void DriveHandler::strafe(int stickTheta, int stickMagnitude) {
    ...
    //forward or backward wheel spin (and the special edge cases) based on joystick angle
    int direction; 
...
float DriveHandler::radialTurnHeadingAngle(int stickAngle) {
    ...
    // derive a percentage for the amount turned (0 - 100% for each quadrant) and multiply by a maximum turning angle
    ...
    // corresponding to a right turn (note the negative reported angle value)
...
float DriveHandler::radialTurnWheelAngles(int headingAngle, bool innerFlag) {
    ...
    float wheelAngle = atan(
        2 * m_drive.getLength() * sin(headingAngle)
        /  (2 * m_drive.getLength() * cos(headingAngle) - wheelInt * m_drive.getWidth()  * sin(headingAngle)));

    return TO_DEGREES(wheelAngle);
}

// Applies to the front wheels regardless of direction
void DriveHandler::radialTurn(int stickAngle, int stickMagnitude, int stickTheta) {
...
void DriveHandler::eventLoop() {
    ...
    while (true) {
        ...
        bool angleVelocityFlag = message.angle_velocity != 0; //right joystick angle
        bool velocityFlag = message.velocity != 0; //left joystick magnitude

        //for if the movement is forward or backward
        //left joystick angle
        bool thetaFlag = (message.theta < 10 && message.theta > 350) || (message.theta < 190 && message.theta > 170);
