...
void Rover::start() {
    ...
    thread driveHandlerThread([&]() {
        driveHandler.star #include "Rover.h"
        ...
        void Rover::start() {
            ...
            Logging::logI(file, "Connecting to WebSocket server at %s:%d",
                          address, WEBSOCKET_PORT);
            WebSocketClient client(address, std::to_string(WEBSOCKET_PORT));

            // Instantiate the systems for the rover
#if EXTENTION == EXTENTION_TYPE_ARM
            const std::vector<MotorType> motorTypes
                = { MOTOR_TYPE_STEPPER_MOTOR, MOTOR_TYPE_DC_MOTOR,
                    MOTOR_TYPE_DC_MOTOR, MOTOR_TYPE_STEPPER_MOTOR,
        ...
        void Rover::stop() {
            // Need to add some functionality to how we want to pause the rover
            // and the handlers (2025-04-12)
        ...
        void Rover::resumeRover() {
            ...
            m_roverConVar.notify_all(); // Notify the condition variable to
                                        // resume the thread
        }
        ...
        void Rover::resumeArmHandler() {
            ...
            m_armConVar.notify_all(); // Notify the condition variable to resume
                                      // the thread
        }
        ...
        void Rover::resumeDriveHandler() {
            ...
            m_driveConVar.notify_all(); // Notify the condition variable to
                                        // resume the thread
        }
...
void Rover::stop() {
        // Need to add some functionality to how we want to pause the rover and
        // the handlers (2025-04-12)
...
void Rover::startClient(MessageQueue* clientQueue, MessageQueue* armQueue,
        ...
        while (true) {
            ...
            default:
                Logging::logE(
                    file, "Recieved message for un-used message format of %d",
                    message.getFormat());
                break;
            }
        }
