...
void ControllerHolder::buttonResponse(Uint8 buttonID, int controllerIndex) {
  ...
  m_controllerList[controllerIndex].getButtonFuncs().buttonArray[buttonID](&buttonName);
}
...
void ControllerHolder::controllerAddedResponse(int controllerIndex) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (!((bool)m_controllerList[i].getPointerID())) {
      m_controllerList[i] = Controller(SDL_GameControllerOpen(controllerIndex));
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_JOYAXISMOTION:
        ...
        if (event.jaxis.axis < 4){
          stickResponse(event.jaxis.value, event.jaxis.axis, event.cdevice.which);
        }
