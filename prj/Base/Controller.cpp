...
std::string getButtonName(Uint8 button) {
    switch (button) {
        case SDL_CONTROLLER_BUTTON_A: return "Cross (A)";
        case SDL_CONTROLLER_BUTTON_B: return "Circle (B)";
        case SDL_CONTROLLER_BUTTON_X: return "Square (X)";
        case SDL_CONTROLLER_BUTTON_Y: return "Triangle (Y)";
        ...
        case SDL_CONTROLLER_BUTTON_LEFTSTICK: return "Left Stick Button";
        case SDL_CONTROLLER_BUTTON_RIGHTSTICK: return "Right Stick Button";
...
//Holds a list of structs that are linked to the positions of the controllers in the list <<<<<<<<<<<<<<<<<<<<<<<
class ControllerHolder {
    ...
    class Stick
    {
        ...
        public:
            ...
            //joysticks are identified by their axes (ex. right stick has a rightX and a rightY axis)
    ...
    public:
        ...
        //the values used to initialize sticks just identify a generic left or right joystick
        Stick m_leftStick = Stick(SDL_CONTROLLER_AXIS_LEFTX, SDL_CONTROLLER_AXIS_LEFTY);
        Stick m_rightStick = Stick(SDL_CONTROLLER_AXIS_RIGHTX, SDL_CONTROLLER_AXIS_RIGHTY);
        ...
        Controller(SDL_GameController* identifier)
        {
            ...
            //joystick / controller instance ID <- joystick pointer <- controller pointer
            m_instanceID = SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(identifier));
    ...
    public:
        ...
        //Attempting to initialize controllers when initializing the controller holder
        //Eventually put the button structs in here

        ControllerHolder(buttonFunctions functionStruct1, buttonFunctions functionStruct2) {
        ...
        void eventLoop(){
            ...
            while (!quit) {
                ...
                //pulls an event from the queue (crucial for updating literally anything!)
                //this is the event loop
                //yet to figure out why event loop has to look like this
                while (SDL_PollEvent( &event ))
                {
                    ...
                    case SDL_CONTROLLERDEVICEADDED:
                        ...
                        //checks for which controller object is not active and connects the new controller there
                        for (int i = 0; i < 3; i++){
                            ...
                            if (!((bool) m_controllerList[i].getPointerID())) {
                                m_controllerList[i] = Controller(SDL_GameControllerOpen(event.cdevice.which));
                                std::cout << i <<std::endl;
                                break;
                            }
                    ...
                    case SDL_CONTROLLERDEVICEREMOVED:
                        ...
                        //checks if there is a controller id and the controller id matches the current controller
                        for (int i = 0; i < 3; i++){
                    ...
                    //Testing may be needed to see if updates can be placed here or I am just dumb
                    case SDL_JOYAXISMOTION:

                        std::cout << "\nAxis motion\n";

                        //axis index has to be converted to be an int to match the enum for the axes
                        //value gets stuck sometimes, can be adjusted with the deadzone
                        std::cout << event.cdevice.which << std::endl << (int) event.jaxis.axis << std::endl << event.jaxis.value << std::endl;
                        break;
