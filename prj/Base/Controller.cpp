...
class ControllerHolder {
    ...
    public:
        // joysticks are identified by their axes (ex. right stick has a rightX
        // and a rightY axis)
    ...
    public:
        ...
        // assigns the controller's identifier upon initializing a controller
        // object
        Controller(SDL_GameController* identifier) {
            ...
            m_instanceID = SDL_JoystickInstanceID(
                SDL_GameControllerGetJoystick(identifier));
        }
...
public:
    ...
    void eventLoop() {
        ...
        while (!quit) {
            ...
            // anything!) this is the event loop yet to figure out why event
            // loop has to look like this
            while (SDL_PollEvent(&event)) {
                ...
                case SDL_CONTROLLERDEVICEADDED:
                    ...
                    // checks for which controller object is not active and
                    // connects the new controller there
                    for (int i = 0; i < 3; i++) {
                ...
                // Testing may be needed to see if updates can be placed here or
                // I am just dumb
                case SDL_JOYAXISMOTION:

                    std::cout << "\nAxis motion\n";

                    // axis index has to be converted to be an int to match the
                    // enum for the axes value gets stuck sometimes, can be
                    // adjusted with the deadzone
                ...
                case SDL_CONTROLLERBUTTONDOWN:
                    ...
                    std::cout << getButtonName(event.cbutton.button)
                              << std::endl;

                    // executes the function according to what controller was
                    // used
